...
class ProjectService:
    ...
    async def update_member_role(
        ...
        if user_id == project.owner_id:
            raise AuthorizationError("Cannot change role of project owner")
    ...
    async def remove_member(
        ...
        if user_id == project.owner_id:
            raise AuthorizationError("Cannot remove the project owner")
    ...
    async def require_member(self, project_id: int, user_id: int) -> None:
        """Raise AuthorizationError if the user is not a member (or owner)."""
        project = await self._projects.get_by_id(project_id)
        if project is None:
            raise NotFoundError("Project not found")
    ...
    async def _require_project_admin(
        ...
        """Raise AuthorizationError unless user is owner or project admin."""
        if project.owner_id == user_id:
            return
        member = await self._projects.get_member(project.id, user_id)
        if member is None or member.role != ProjectMemberRole.ADMIN:
            raise AuthorizationError("You do not have admin access to this project")
