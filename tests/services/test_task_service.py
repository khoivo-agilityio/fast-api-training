...
class TestTaskService(unittest.TestCase):
    ...
    def test_add_task_with_custom_status(self) -> None:
        ...
        task = self.service.add_task("Test Task", status=TaskStatus.TODO)
    ...
    def test_list_by_status_filters_correctly(self) -> None:
        ...
        self.service.add_task("Task 1", status=TaskStatus.BACKLOG)
        self.service.add_task("Task 2", status=TaskStatus.TODO)
        self.service.add_task("Task 3", status=TaskStatus.TODO)
    ...
    def test_update_task_title(self) -> None:
        ...
        updated = self.service.update_task(task.id, title="New Title")
    ...
    def test_update_task_description(self) -> None:
        ...
        task = self.service.add_task("Test", "Original")

        updated = self.service.update_task(task.id, description="New Description")
    ...
    def test_update_task_status_valid_transition(self) -> None:
        ...
        updated = self.service.update_task(task.id, status=TaskStatus.TODO)
    ...
    def test_update_task_status_invalid_transition(self) -> None:
        ...
        task = self.service.add_task("Test")  # BACKLOG

        with self.assertRaises(ValueError) as context:
            self.service.update_task(task.id, status=TaskStatus.DONE)
    ...
    def test_mark_as_done_success(self) -> None:
        ...
        task = self.service.add_task("Test")
        self.service.update_task(task.id, status=TaskStatus.TODO)
        self.service.update_task(task.id, status=TaskStatus.IN_PROGRESS)
        self.service.update_task(task.id, status=TaskStatus.TESTING)
    ...
    def test_get_summary_with_tasks(self) -> None:
        ...
        self.service.add_task("Task 1", status=TaskStatus.BACKLOG)
        self.service.add_task("Task 2", status=TaskStatus.TODO)
        self.service.add_task("Task 3", status=TaskStatus.IN_PROGRESS)
        task4 = self.service.add_task("Task 4", status=TaskStatus.IN_PROGRESS)
